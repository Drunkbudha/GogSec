...
def ExecuteCrawler(url):
    execute = str('gnome-terminal -e \' sudo python3 Modules/Crawler/crawl.py ' + url + '\'')
    os.system(execute)

# Terminal Process for Scraper (Only for Gnome at the moment).
def ExecuteScraper(url):
    execute = str('gnome-terminal -e \' sudo python3 Modules/Scraper/Scrape.py ' + url + '\'')
    print (execute)
    os.system(execute)
